...
async function serveFresh ({ file, mapping, reply, params, fname, ext }) {
  const { get } = this.app.lib._
  const { fs, sprintf } = this.app.lib
  const { fetchUrl } = this.app.bajoExtra
  const dir = path.dirname(file)
  fs.ensureDirSync(dir)
  let remoteUrl = mapping.remote ?? get(mapping, '_ref.group.remote')
  remoteUrl = remoteUrl
    .replace('{s}', this.getSubDomains(mapping.subDomains || get(mapping, '_ref.group.subDomains') || undefined))
    .replace('{variant}', mapping.variant || get(mapping, '_ref.group.variant') || '')
    .replace('{size}', mapping.size || get(mapping, '_ref.group.size') || '256')
    .replace('{apiKey}', mapping.apiKey || get(mapping, '_ref.group.apiKey') || '')
  ...
  await serve.call(this, { file, mapping, reply })
}
